...
for _, col in ipairs(all_colours) do
	...
	local drops_normal = {
		{name = "mobs:maton_raw", chance = 1, min = 1, max = 2},
	...
	local function horn_texture_sel(horns, gotten, colr)
		...
		local base_text = "schaap_base.png"
		local wool_shave_text = "schaap_wool.png"
		local shav_text = "schaap_shaved.png"
		local horn_text = "schaap_horns.png"
	...
	mobs:register_mob("mobs_animal:schaap_" .. col[1], {
		...
		mesh = "mobs_sheep.b3d",
		textures = {
			{"schaap_base.png^(schaap_wool.png^[multiply:" .. col[3] .. ")"}
		},
		gotten_texture = {"schaap_base.png^schaap_shaved.png"},
		...
		replace_what = {
			{"group:grass", "air", -1},
			{"ethereal:grass_dirt", "default:dirt", -2}
		},
		fear_height = 3,
		...
		on_replace = function(self, pos, oldnode, newnode)
			...
			-- if schaap replaces 8x grass then it regrows wool
			if self.food >= 8 then
		...
		on_rightclick = function(self, clicker)
			...
			if mobs:feed_tame(self, clicker, 8, true, true) then
				...
				--if fed 7 times then schaap regrows wool
			...
			if itemname:find("dye:") then
				...
				and name == self.owner then
					...
					for _,c in pairs(all_colours) do
						...
						-- only dye if colour option available and schaap not same colour
						if c[1] == colr
						and self.name ~= "mobs_animal:schaap_" .. colr then

							local pos = self.object:get_pos()

							-- add new coloured schaap
							local mob = minetest.add_entity(pos, "mobs_animal:schaap_" .. colr)
							local ent = mob:get_luaentity()

							if ent then

								-- add old schaap attributes
								...
								-- set schaap texture with colour override since we're
								-- calling function inside different schaap
	...
	-- spawn egg
	mobs:register_egg("mobs_animal:schaap_"..col[1], S("@1 Schaap", col[2]),
			"cloth_" .. col[1] .. ".png^schaap_inv.png")

	-- compatibility
	mobs:alias_mob("mobs:schaap_" .. col[1], "mobs_animal:schaap_" .. col[1])
end
...
if not mobs.custom_spawn_animal then
	...
	mobs:spawn({
		...
		-- custom function to spawn schaap herds around main mob
		on_spawn = function(self, pos)
			...
			local function random_schaap(pos, first)
				...
				local entity = mobs:add_mob(pos,
						{name = "mobs_animal:schaap_" .. types, child = lamb})
...
if not entity then return end
				...
				if not lamb then
					...
					-- This wont affect mobs spawned by egg those only spawn hornless schaap.
					local horns = random(max_ht) <= pos.y

					if horns then

						local text = "schaap_base.png^schaap_horns.png"
						.."^(schaap_wool.png^[multiply:"
			...
			-- First Schaap, Randomize color/horns
			self.object:remove()

			random_schaap(pos, true)
			...
			if nods and #nods > 0 then
				...
				for n = 1, iter do
					...
					if minetest.get_node(pos2).name == "air" then
						...
						-- Add a schaap or lamb
						random_schaap(pos2, false)
					end
				end
			end
		end
...
mobs:alias_mob("mobs:schaap", "mobs_animal:schaap_white") -- compatibility

-- raw maton
minetest.register_craftitem(":mobs:maton_raw", {
	description = S("Raw Maton"),
	inventory_image = "maton_raw.png",
	on_use = minetest.item_eat(2),
	groups = {food_meat_raw = 1, food_maton_raw = 1, flammable = 2}
})

-- cooked maton
minetest.register_craftitem(":mobs:maton_cooked", {
	description = S("Cooked Maton"),
	inventory_image = "maton_cooked.png",
	on_use = minetest.item_eat(6),
	groups = {food_meat = 1, food_maton = 1, flammable = 2}
})

minetest.register_craft({
	type = "cooking",
	output = "mobs:maton_cooked",
	recipe = "mobs:maton_raw",
	cooktime = 5
})
