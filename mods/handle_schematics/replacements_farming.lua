...
replacements_group['farming'].add_material = function( fruit, fruit_item, prefix, seperator, postfix  )
	...
	if( is_loaded and minetest.get_modpath('mobf_trader') and mobf_trader and mobf_trader.add_trader ) then
		...
		local goods = {
			{ fruit_item.." 1",   "ores:coal_lump 3",    "wood:tavifruit_wood 8"},
			{ fruit_item.." 10",  "ores:ferrum_ingot 2",  "wood:chest_wood_locked 1"}};
		if( fruit_item ~= data[1] ) then
			table.insert( goods, { data[1].." 1", "farming:scarecrow", "farming:scarecrow_light 1"});
			table.insert( goods, { data[1].." 2", "default:dirt 20", "default:bucket_water", "ores:ferrum_ingot 4", "wood:tavifruit_leaves 99" });
		end
		table.insert( goods, {"farming:hoe_wood 1","wood:tavifruit_wood 10", "default:cobble 10"});
...
replacements_group['farming'].construct_farming_type_list = function()
	...
	replacements_group['farming'].add_material( 'rice',   'farming:rice',                    'farming:', '_', '' );
