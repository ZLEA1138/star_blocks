...
local function generate_building_translate_nodenames( nodenames, replacements, cid, binfo_scm, mirror_x, mirror_z )
	...
	for i,node_name in ipairs( nodenames ) do
		...
		elseif(     node_name == 'wood:chest_wood'
		   or   new_node_name == 'wood:chest_wood' ) then
			new_nodes[ i ].is_replaced   = 1; -- currently unused
			new_nodes[ i ].special_chest = 'wood:chest_wood';
			new_nodes[ i ].new_content   = cid.c_chest;
		...
		if( new_node_name and regnode) then
			...
			if(    node_name == 'wood:ladder_wood' and handle_schematics.is_realtest) then
...
handle_schematics.generate_building = function(pos, minp, maxp, data, param2_data, a, extranodes, replacements, cid, extra_calls, building_nr_in_bpos, village_id, binfo_extra, road_node, keep_ground, scaffolding_only)
	...
	local c_dirt_with_grass      = GCIR( "ethereal:grass_dirt", replacements );
	local c_dirt_with_snow       = GCIR( "ethereal:snow_dirt", replacements );
...
handle_schematics.place_buildings = function(village, minp, maxp, data, param2_data, a, cid, village_id)
	...
	cid.c_chest            = GCIR( 'wood:chest_wood',          replacements );
	cid.c_bookshelf        = GCIR( 'default:bookshelf',      replacements );
	cid.c_chest_locked     = GCIR( 'wood:chest_wood_locked',   replacements );
	...
	cid.c_sign             = GCIR( 'wood:sign_wall_wood',      replacements );
...
handle_schematics.get_cid_table = function( replacements )
	...
	cid.c_dirt_with_grass  = GCIR( 'ethereal:grass_dirt',replacements );
	...
	cid.c_snowblock        = GCIR( 'default:snow_block', replacements);
	cid.c_dirt_with_snow   = GCIR( 'ethereal:snow_dirt', replacements );
	...
	cid.c_fwtree           = GCIR( 'wood:ferrumwood_trunk', replacements);
	cid.c_mtree            = GCIR( 'wood:marshtree_trunk', replacements);
	cid.c_otree            = GCIR( 'wood:oki_trunk', replacements);
	cid.c_patree           = GCIR( 'wood:palm_trunk', replacements);
	cid.c_pitree           = GCIR( 'wood:pine_tree', replacements);
	cid.c_satree           = GCIR( 'wood:sana_trunk', replacements);
	cid.c_sotree           = GCIR( 'wood:scorched_tree', replacements);
	cid.c_tftree           = GCIR( 'wood:tavifruit_trunk', replacements);
	cid.c_water            = GCIR( 'default:water_source', replacements); -- PM ^
	cid.c_stone_with_coal  = GCIR( 'ores:stone_with_coal', replacements);
