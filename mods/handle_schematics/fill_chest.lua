...
ADD_RCC({"ores:aurum_block",               90, 99, 4, wreck=1});
ADD_RCC({"ores:wolfram_block",             80, 10, 2, wreck=1});
ADD_RCC({"ores:cuprum_block",              95, 99, 5, wreck=1});
ADD_RCC({"ores:stannum_block",             95, 99, 5, wreck=1});
ADD_RCC({"ores:aereus_block",              95, 99, 5, wreck=1});
ADD_RCC({"ores:adamas_crystal_green",      70, 60, 2, wreck=1});
ADD_RCC({"ores:adamas_crystal_blue",       70, 60, 2, wreck=1});
ADD_RCC({"ores:ferrum_ingot",              80, 99, 3, wreck=1});
ADD_RCC({"ores:cuprum_ingot",              80, 99, 5, wreck=1});
ADD_RCC({"ores:stannum_ingot",             80, 99, 4, wreck=1});
ADD_RCC({"ores:aereus_ingot",              90, 99, 4, wreck=1});
ADD_RCC({"ores:wolfram_ingot",             90, 99, 2, wreck=1});

ADD_RCC({"default:pick_stone",             10,  1, 3, farm_tiny=1, farm_full=1, shed=1, lumberjack=1, hut=1, chest_work=1, lumberjack=1 }); 
ADD_RCC({"ores:pick_ferrum",                5,  1, 2, forge=1 }); 
ADD_RCC({"ores:pick_wolfram",               2,  1, 2, forge=1, lumberjack=1 }); 
ADD_RCC({"default:shovel_stone",            5,  1, 3, farm_tiny=1, farm_full=1, shed=1, lumberjack=1, hut=1, chest_work=1 }); 
ADD_RCC({"ores:shovel_ferrum",              5,  1, 2, forge=1 }); 
ADD_RCC({"default:axe_stone",               5,  1, 3, farm_tiny=1, farm_full=1, chest_work=1, lumberjack=1 }); 
ADD_RCC({"ores:axe_ferrum",                 5,  1, 2, forge=1, lumberjack=1 }); 
ADD_RCC({"default:sword_wood",              1,  1, 3, guard=1, ship=1 });
ADD_RCC({"default:sword_stone",             1,  1, 3, guard=1 });
ADD_RCC({"ores:sword_ferrum",               1,  1, 3, forge=1, guard=1, ship=1 });

ADD_RCC({"wood:stick",                     20, 40, 2, church=1, library=1, chest_private=1, shelf=5, shed=1, lumberjack=1, hut=1, ship=1 });
...
ADD_RCC({"wood:tavifruit",                 50, 10, 2, chest_storage=4, chest_private=1, shelf=5, ship=1});
ADD_RCC({"wood:ladder_wood",               20,  1, 2, church=1, library=1, shed=1, lumberjack=1, hut=1, ship=1 });
        
ADD_RCC({"ores:coal_lump",                 80, 30, 1, forge=1, shed=1, lumberjack=1, hut=1, ship=1});
ADD_RCC({"ores:ferrum_lump",               30,  4, 2, forge=1, ship=1 });
ADD_RCC({"ores:adamas_crystal_red",        10,  3, 1, forge=1, chest_storage=1, ship=1 });
...
ADD_RCC({"vessels:ferrum_bottle",          10,  1, 1, church=1, library=1, shelf=1 });
...
ADD_RCC({"ores:aurum_ingot",                1,  2, 1 });
ADD_RCC({"ores:ferrum_ingot",               1,  2, 1 });
ADD_RCC({"ores:cuprum_ingot",              30, 10, 1 });
ADD_RCC({"ores:stannum_ingot",              1,  5, 1 });
ADD_RCC({"ores:aereus_ingot",               1,  1, 1 });
ADD_RCC({"ores:wolfram_ingot",              1,  1, 1 });
...
ADD_RCC({'simplecoins:coin_federation',              80, 10,  2, chest_private=1, chest_work=1, ship=1 }); -- TODO: could be in any chest with a certain chance
...
handle_schematics.fill_chest_random = function( pos, pr, building_data_typ )
	...
	if(     typ == 'default:bookshelf') then
		typ = 'chest_bookshelf';
		inv_source = 'books';
	elseif( typ == 'wood:chest_wood') then
		typ = 'chest_default';
